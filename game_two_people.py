class Game:
    ...
    def play(self):
        ...
        # устанавливаем корабли на доску
        ...
        while not winner:
            if first_players_turn:
                ...
                if winner:
                    print(f"Игра окончена! Победил(а) {name1}!")
            else:
                winner = self.players[1].take_turn()
                if winner:
                    print(f"Игра окончена! Победил(а) {name2}!")
...
class Board:
    ...
    # вывод игровой доски с кораблями
    def __str__(self):
        str_val = "    1 | 2 | 3 | 4 | 5 | 6 |\n"
        for i in range(6):
            str_val += f"{i + 1} |"
    ...
    # вывод игровой доски противника без кораблей
    ...
    # добавляем корабль на доску без пересечений и соприкосновений с другими кораблями
    def add_boat(self, boat):
        ...
        for x in range(-1, width + 1):
            for y in range(-1, height + 1):
                if boat.y + y >= 0 and boat.y + y < 6 and boat.x + x >= 0 and boat.x + x < 6 and self.grid[boat.y + y][
                    boat.x + x] != ' O |':
                    return False
    ...
    # отмечаем выстрел на доске и считаем успешные выстрелы
    def attack(self, x, y):
    ...
    # проверяем, не является ли выстрел повторным
    def is_repeated(self, x, y):
        if self.grid[y][x] != ' O |' and self.grid[y][x] != ' ■ |':
            return True

    # проверяем, потоплены ли все корабли (11 успешных выстрелов)
...
class Boat:
    ...
    def __init__(self, label, size):
        ...
        # устанавливаем координаты первой палубы корабля

    def set_position(self, x, y):
        self.x = x
        self.y = y

    # устанавливаем ориентацию корабля (горизонтальная или вертикальная)
...
class Player:
    ...
    def __init__(self, name):
        self.name = name
        self.board = Board()
        self.fleet = [Boat("первый корабль", 3), Boat("второй корабль", 2), Boat("третий корабль", 2),
                      Boat("четвертый корабль", 1), \
    ...
    # устанавливаем на доску все корабли по списку
    def position_fleet(self):
        print(f"{self.name}, давайте расположим ваши корабли на игровой доске.")

        for boat in self.fleet:
            self.position_boat(boat)

        print(f"{self.name}, ваш флот готов к игре!")
        print("\n")

    # правильно позиционируем корабль на доску
    def position_boat(self, boat):
        ...
        while position is None:
            try:
                position = input(
                    "Введите координаты первой палубы корабля через запятую: сначала номер столбца, затем номер строки. ")
    ...
    # игрок делает выстрел по доске противника (компьютера)
    def take_turn(self):
        print(f"{self.name}, это ваша доска:")
        print(self.board)
        print()

        while True:
            print(f"Это доска противника ({self.opponent.name}):")
...
name1 = input('Введите имя первого игрока и нажмите ENTER: ')
name2 = input('Введите имя второго игрока и нажмите ENTER: ')
